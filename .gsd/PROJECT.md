...
The remaining near-term trust gap is no longer raw rollout freshness alone; S09 moved the real rollout refs and proved exactly which hosted lanes still block the assembled release proof. `main`, `v0.1.0`, and `ext-v0.3.0` now all point at the repaired rollout SHA `8e6d49dacc4f4cd64824b032078ae45aabfe9635`, and the remote-evidence freshness gate in `scripts/verify-m034-s05.sh` now records expected vs observed `headSha` values so stale-green runs cannot masquerade as current proof. On that rolled-out SHA, `deploy.yml`, `deploy-services.yml`, `extension-release-proof.yml`, and `publish-extension.yml` are green on the correct `headSha`, so the repo-side Docker/image, duplicate-safe extension publish, and hosted extension/deploy reroll repairs are all carrying their weight.

What is still red is narrower and better isolated: `authoritative-verification.yml` fails on the correct `headSha` because `scripts/verify-m034-s01.sh` still sees package-level `latest` metadata lag the just-published proof version, and `release.yml` fails on the same `headSha` because the Windows staged installer verifier still stops at `installed meshc.exe build installer smoke fixture failed` even after the strict-mode `$LASTEXITCODE` fix. The canonical stop-after replay (`VERIFY_M034_S05_STOP_AFTER=remote-evidence bash scripts/verify-m034-s05.sh`) now fails for those two hosted reasons, not for stale refs, and `.tmp/m034-s06/evidence/first-green/` must remain absent until those lanes rerun green. M034 is therefore in a closeout state where public freshness gating and hosted evidence are mechanically honest, but the milestone is still blocked on registry-latest propagation and the remaining Windows release-smoke failure.
